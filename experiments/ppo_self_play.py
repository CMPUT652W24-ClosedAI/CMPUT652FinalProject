        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                if args.ai:
                    ...
                    # Did not work here
                    p2_action_reversed = torch.flip(p2_action, dims=[1])
                    # p2_action_reversed[:, :, 1:5] = (p2_action_reversed[:, :, 1:5] + 2) % 4
                    p2_action_reversed[:, :, 1:5].add_(2).remainder_(4)

                    action = torch.zeros(
                        (args.num_envs, p1_action.shape[1], p2_action_reversed.shape[2])
                    )
                    action[::2] = p1_action
                    action[1::2] = p2_action_reversed
